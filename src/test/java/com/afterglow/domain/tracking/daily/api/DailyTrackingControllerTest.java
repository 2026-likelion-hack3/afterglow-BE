...
class DailyTrackingControllerTest {
	...
	void 수면과_컨디션을_기록하면_204를_받는다() throws Exception {
		...
		DailyTrackingRequest request = new DailyTrackingRequest(
			...
			com.afterglow.domain.tracking.daily.domain.SleepLevel.POOR,
			com.afterglow.domain.tracking.daily.domain.ConditionLevel.NORMAL,
			37.5665,
			126.9780
		);
	...
	void 같은_날짜에_다시_기록하면_기존_기록을_수정한다() throws Exception {
		...
		DailyTrackingRequest firstRequest = new DailyTrackingRequest(
			LocalDate.of(2026, 8, 16),
			com.afterglow.domain.tracking.daily.domain.SleepLevel.POOR,
			com.afterglow.domain.tracking.daily.domain.ConditionLevel.BAD,
			37.5665,
			126.9780
		);
		...
		DailyTrackingRequest secondRequest = new DailyTrackingRequest(
			LocalDate.of(2026, 8, 16),
			com.afterglow.domain.tracking.daily.domain.SleepLevel.WELL,
			com.afterglow.domain.tracking.daily.domain.ConditionLevel.GOOD,
			37.5665,
			126.9780
		);
