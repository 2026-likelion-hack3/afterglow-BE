...
- **날씨 "일치" 판정 규칙 확정(2026-08-20)**: 날씨 조건 충족+`WORSE`/`SAME` → 일치, 조건 충족+`IMPROVED` → 불일치. 조건 미충족+`IMPROVED` → 일치, 조건 미충족+`SAME`/`WORSE` → 불일치(`WeatherMatchRule`). **"날씨 조건 충족(conditionMet)" threshold도 확정(2026-08-21)** — 아래 세 그룹 중 하나라도 충족하면 OR: (1) 평균습도 40% 미만 또는 전날 대비 평균습도 15%p 이상 하락, (2) 전날 대비 평균기온 5°C 이상 하락 또는 당일 최저기온 5°C 미만, (3) 당일 또는 전날 최대 UV 6 이상(`WeatherConditionRule`). Weather candidate가 실제로 활성화됐다 — 기존 분석 우선순위/다른 candidate 로직은 변경하지 않았다.
...
- **RC1 CONTINUE_USE 제품 선정 정책(2026-08-20 신설 — 기존 로직에서 발견한 게 아니라 이번에 새로 확정한 최소 규칙, AI 미사용)**: Routine/Analysis 어디에도 "어떤 보유 제품을 계속 써도 되는지" 판단하는 기존 deterministic 로직이 없었다(Routine은 사용자가 고른 값을 그대로 저장할 뿐 positive selection을 하지 않는다, `RoutinePlanner` 참고) — 그래서 "원인이 아닌 나머지 전체"를 보여주는 방식은 근거 없는 추천으로 판단해 폐기했다. 대신: **HOLD면 항상 빈 배열. HOLD가 아니어도 top 원인이 PRODUCT/COMBINATION이 아니면(SLEEP/WEATHER) 항상 빈 배열.** 그 외의 경우만, 계정 Vanity 보유 제품 중 `InteractionTag.LOW_IRRITATION`이 명시적으로 있고 RETINOL/ACID/VITAMIN_C/HIGH_CONCENTRATION이 전혀 없으며, 이번 분석의 원인 제품(들)이 아니고, Routine 이력상 현재 중단 상태도 아닌 제품만 포함한다(여러 개면 전부 반환, 없으면 빈 배열 — 빈 배열은 오류가 아니다). LOW_IRRITATION은 OpenAI/OCR로 새로 추론하지 않고 Product에 이미 저장된 태그만 신뢰한다. 구현: `EpisodeAnalysisService.withContinueUseProducts`.
...
- ~~(2026-08-18 신규) 날씨(온도/습도/자외선) matching threshold~~ — **2026-08-21 확정, 해소.** "일치" 판정 규칙(`WeatherMatchRule`)과 "조건 충족" threshold(`WeatherConditionRule`) 모두 확정돼 Weather candidate가 활성화됐다. 위 "날씨 '일치' 판정 규칙 확정" 항목 참고.
