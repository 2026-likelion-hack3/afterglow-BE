...
- **스키마**: migration 도구(Flyway/Liquibase)가 아직 없어 prod의 `ddl-auto: validate`는 스키마가 이미 존재해야만 정상 기동한다. **완전히 빈 신규 DB에 최초로 배포할 때만** 별도 절차로 스키마를 만들고 `validate`로 정상 기동되는 것까지 확인했다 — 이 절차는 반복 가능한 배포 방법이 아니므로 여기 문서화하지 않는다. Flyway 도입은 CD를 붙이는 시점에 반드시 선행되어야 할 별도 작업이다.
...
- **AWS EC2/RDS 수동 배포 및 스모크 테스트 완료**: health check, Account/Episode API, 실제 SES 이메일 발송까지 전부 정상 동작을 확인했다. RDS/EC2 프로비저닝 자동화(IaC), GHCR push, CD 파이프라인, SES production access, Flyway 도입, IAM Identity Center 전환은 이후 별도 작업으로 진행한다.
